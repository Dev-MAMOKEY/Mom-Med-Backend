...
CREATE TABLE IF NOT EXISTS app.users (
    -- 내부 식별자입니다. pgcrypto의 gen_random_uuid()를 사용해 애플리케이션 밖에서도 안전하게 생성합니다.
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 로그인, 알림, 운영 식별에 사용할 이메일입니다. MVP에서는 smoke key 역할도 합니다.
    email VARCHAR(255) NOT NULL,

    -- 화면에 표시할 사용자 이름입니다. 아직 인증 정책이 확정되지 않았으므로 nullable로 둡니다.
...
-- 최신 가입자 조회나 smoke 확인 쿼리를 빠르게 하기 위한 보조 인덱스입니다.
CREATE INDEX IF NOT EXISTS idx_users_created_at ON app.users (created_at DESC);

-- updated_at 자동 갱신 trigger입니다. V003의 공통 함수를 재사용합니다.
