package mamokey.mom_med.backend.external.mfds;

import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * 식약처 제품 허가정보 상세 API의 item입니다.
 */
public record MfdsDrugDetailItem(
		@JsonProperty("ITEM_SEQ") String itemSeq,
		@JsonProperty("ITEM_NAME") String itemName,
		@JsonProperty("ITEM_NAME_ENG") String itemNameEng,
		@JsonProperty("ENTP_NAME") String entpName,
		@JsonProperty("ENTP_NO") String entpNo,
		@JsonProperty("ITEM_PERMIT_DATE") String itemPermitDate,
		@JsonProperty("SPCLTY_PBLC") String specialtyType,
		@JsonProperty("EDI_CODE") String ediCode,
		@JsonProperty("ATC_CODE") String atcCode,
		@JsonProperty("MAIN_INGR_ENG") String mainIngrEng,
		@JsonProperty("CHART") String chart,
		@JsonProperty("NB_DOC_DATA") String nbDocData,
		@JsonProperty("EE_DOC_DATA") String eeDocData,
		@JsonProperty("UD_DOC_DATA") String udDocData,
		@JsonProperty("CHANGE_DATE") String changeDate
) {
}
